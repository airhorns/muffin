(function() {
  var CoffeeScript, ask, clocFile, clocPath, compileMap, compileScript, copyFile, doccoFile, ensurePerl, exec, extend, fs, getGitRoot, glob, growlAvailable, growlCheckPromise, growlCommand, handleFileError, inRebase, k, langDefPath, minifyScript, notify, ofs, orgExec, path, perlError, perlPresent, printTable, q, readFile, run, runOptions, spawn, statFile, statFiles, temp, v, writeFile, _, _ref, _ref2, _ref3, _statFiles;
  ...
  writeFile = function(file, data, options) {
    ...
    if (runOptions.commit) {
      ...
      write = q.defer();
      ofs.writeFile(file, data, "UTF-8", function(err) {
        if (err) {
  ...
  compileScript = function(source, target, options) {
    ...
    return readFile(source, options).then(function(data) {
      ...
      try {
        ...
        return q.when(writeFile(target, js, options), function(whatevs) {
          if (options.notify !== false) {
  ...
  run = function(args) {
    ...
    q.when(start = before(), function() {
      ...
      compiledMap.forEach(function(map) {
        var file, i, matches, work, _ref3, _results;
        _ref3 = args.files;
        _results = [];
        for (i in _ref3) {
          file = _ref3[i];
          _results.push((function() {
            if (matches = map.pattern.exec(file)) {
              ...
              work = q.defer();
              try {
                q.when(map.action(matches), (function(data) {
                  return work.resolve(data);
                }), (function(reason) {
                  return work.reject(reason);
                }));
              } catch (e) {
                work.reject(e);
              }
                ...
                return (function(map, matches) {
                  return ofs.watchFile(file, {
                    ...
                    q.when(subStart = before(), function() {
                      ...
                      q.when(moreWork, function(result) {
                        if (args.after) {
                          return args.after();
                        }
                      });
      ...
      return done = q.when(done, function(v) {
        if (args.after) {
          args.after();
        }
        return true;
