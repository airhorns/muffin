(function() {
  ...
  minifyScript = function(source, options) {
    ...
    return readFile(source, options).then(function(original) {
      var finalCode, finalPath, sq, stream, toplevel;
      toplevel = UglifyJS.parse(original, {
        filename: source
      });
      toplevel.figure_out_scope();
      if (options.transform != null) {
        toplevel = options.transform(toplevel, UglifyJS);
      }
      sq = UglifyJS.Compressor({
        warnings: false,
        global_defs: options.defines
      });
      ...
      finalCode = stream + "";
      return writeFile(finalPath.join('.'), finalCode, options).then(function() {
        return finalCode;
      });
    });
  };
  ...
  _statFiles = function(files, options) {
    ...
    promises = files.map(function(file) {
      return q.spread([clocFile(file), statFile(file)], function(clocstats, filestats) {
          ...
          if (matches = map.pattern.exec(file)) {
            ...
            if (args.options.watch) {
              ...
              _fn = function(map, matches, file) {
                var lastTime, queueCompile, timeout, watch, watcher,
                  _this = this;
                timeout = null;
                watcher = null;
                lastTime = 0;
                queueCompile = function() {
                  var compile,
                    _this = this;
                  clearTimeout(timeout);
                  compile = function() {
                    return ofs.stat(file, function(err, stats) {
                      if (err) {
                        return console.error(err);
                      }
                      if (stats.mtime.getTime() === lastTime) {
                        return watch();
                      }
                  ...
                  return timeout = setTimeout(compile, 20);
                };
                return (watch = function() {
                  if (watcher != null) {
                    watcher.close();
                  }
                  return watcher = ofs.watch(file, {
                    persistent: true
                  }, function(event) {
                    if (!inRebase()) {
                      return queueCompile();
